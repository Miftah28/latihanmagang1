...
@section('main-content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <!-- Vertical Form -->
            <form class="row g-3" action="{{ route('admin.penumpang.update', Crypt::encrypt($data->id)) }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="col-12">
                    <label for="inputNanme4" class="form-label">Your Name</label>
                    <input id="nama" type="text" class="form-control @error('nama') is-invalid @enderror"
                        name="nama" value="{{ $data->name }}" required autocomplete="name" autofocus >
                    @if ($errors->has('nama'))
